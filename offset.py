...
for place in places:
	Path(f'./output/{place}/offset').mkdir(parents=True, exist_ok=True)

	for i in tqdm(range(3, 15), f'  Parsing Offset ({place})'):
		output = open(f'./output/{place}/offset/{i-2}.txt', 'w', encoding='utf-8')

		file = open(f'./pages/{place}/offset/page-{i}.txt', 'r', encoding='utf-8')
